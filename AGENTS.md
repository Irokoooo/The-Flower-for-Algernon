...
Track every player-requested refinement with a stable ID in `docs/OPTIMIZATION_TODO.md`; the lightweight board reads this file directly. Keep implementation status distinct from human acceptance. Preserve deferred items and update evidence when work lands; prioritize playable scene progression before unrequested polish.
See `docs/PLAYTEST_FEEDBACK.md`. The user subsequently activated the original illustrated test stimulus, licensed book/diary GLB reuse, and diary handwriting/dragging/audio polish. Diary fragments have phase-specific vocabulary, authored homophone errors and uneven handwriting, with visible drag placement and stable controls. Preserve prior saved writing unchanged. Physical bread pickup remains deferred; its recorded follow-up requires actual pickup/place with hands rather than a counter button. Test image stays objectively unchanged across cognition phases.
