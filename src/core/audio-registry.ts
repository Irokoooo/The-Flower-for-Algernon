...
const files = import.meta.glob('../../assets/audio/**/*.{wav,mp3}', {eager:true,query:'?url',import:'default'}) as Record<string,string>;
export function resolveAudioAsset(id:string): AudioAsset {
 ...
 if (entry.kind === 'voice') {
   if (!('language' in entry) || entry.language !== 'en' || !('voiceProfileId' in entry) || typeof entry.voiceProfileId !== 'string') {
     throw Error(`English voice metadata missing: ${id}`);
   }
   return {id,url,kind:'voice',language:'en',voiceProfileId:entry.voiceProfileId,provenance:entry.provenance as AudioAsset['provenance']};
 }
