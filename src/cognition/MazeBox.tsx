...
export interface MazeBoxProps { cognition: CognitionState; algernonPresent?: boolean; suspended?: boolean; className?: string; onInspectChange?: (inspecting: boolean) => void }
// Each rectangle leaves at least .35 units around the entire authored route.
const WALLS = [[-1,1.25,.13,1.5],[1,0,.13,3],[-1,-.5,2.8,.12],[-1,-1.5,.9,.12]];
export function MazeBox({ cognition, algernonPresent = true, suspended = false, className = '', onInspectChange }: MazeBoxProps) {
  ...
  const view = useRef({tilt:0,rotation:0,zoom:1});
  ...
  useEffect(()=> {
    ...
    const ctx=grain.getContext('2d')!; ctx.fillStyle='#b58a56';ctx.fillRect(0,0,512,512);
    // Deterministic procedural timber, no remote textures or undocumented assets.
    for(let i=0;i<370;i++) {ctx.strokeStyle=`rgba(${i%3===0?'63,34,16':'238,197,139'},${.035+(i%7)*.01})`;ctx.lineWidth=.4+i%3*.3;ctx.beginPath();for(let x=0;x<=512;x+=8){const y=i*1.4+Math.sin(x*.012+i*.7)*2+Math.sin(x*.038+i)*.7;if(x===0)ctx.moveTo(x,y);else ctx.lineTo(x,y);}ctx.stroke();}
    const texture=new THREE.CanvasTexture(grain);texture.colorSpace=THREE.SRGBColorSpace;texture.wrapS=texture.wrapT=THREE.RepeatWrapping;texture.anisotropy=renderer.capabilities.getMaxAnisotropy();
    const mat=(color:number,wood=false)=> {const m=new THREE.MeshStandardMaterial({color,roughness:wood?.62:.8,map:wood?texture:null});materials.push(m);return m;};
    const timber=mat(0xe9c693,true),edge=mat(0x936641,true),fur=mat(0xf7f2e5),pink=mat(0xc9968e),eye=mat(0x221b18),brass=mat(0xba9450),foodMat=mat(0xc69a54);
    ...
    box(0,-.22,0,5.65,.4,5.65,edge);box(0,0,0,5.2,.06,5.2);
    ...
    for(const [x,z,w,d] of WALLS) {box(x,.24,z,w,.44,d);box(x,.47,z,w+.025,.025,d+.025);}
    ...
    const ground=box(0,-.46,0,200,.05,200,mat(0x262821));ground.castShadow=false;
    ...
    eye.roughness=.16;
    const body=ellipsoid(0,.16,0,.14,.13,.24,fur);
    const head=ellipsoid(0,.18,.22,.10,.09,.13,fur);ellipsoid(0,.16,.335,.036,.029,.035,pink);
    ...
    const animate=(now:number)=> {
      ...
      if(interaction) {
        ...
        body.scale.y=.13+(t>.8 && t<1.8?Math.sin(t*15)*.006:0);
      ...
      const distance=Math.max(10,7/Math.max(.4,camera.aspect))/zoom;
    ...
    return ()=> {cancelAnimationFrame(frame);observer.disconnect();pick.current=()=>{};offer.current=()=>{};geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());texture.dispose();renderer.dispose();renderer.domElement.remove();};
  },[]);
  ...
  const zoom=(amount:number)=> {view.current.zoom=Math.max(.7,Math.min(1.65,view.current.zoom+amount));};
  const reset=()=> {view.current={tilt:0,rotation:0,zoom:1};};
  return <div hidden={suspended} className={`cognition-maze-shell ${expanded?'is-expanded':''}`}
    ...
    <aside ref={panel} className={`cognition-maze ${className}`} role={expanded?'dialog':undefined} aria-modal={expanded?true:undefined} aria-label="Algernon's maze / 阿尔吉侬的迷宫">
      <header><div><small>OBSERVATION BOX · 01</small><h2>Algernon <span lang="zh-Hans">阿尔吉侬</span></h2></div>
        <button ref={expandButton} type="button" onClick={()=>setExpanded(v=>!v)}>{expanded?'Close / 关闭':'Inspect / 放大'}</button></header>
      ...
      <div className="cognition-maze__tools"><button type="button" onClick={()=>zoom(-.15)} aria-label="Zoom out / 缩小">−</button><button type="button" onClick={()=>zoom(.15)} aria-label="Zoom in / 放大">+</button><button type="button" onClick={reset}>Top view / 俯视</button><button type="button" disabled={!algernonPresent || error || interacting} onClick={()=>offer.current()}>Offer grain / 喂食</button></div>
      <p role="status">{!algernonPresent?'The box is quiet. / 盒子安静了。':interacting?'A moment of curiosity. / 片刻的好奇。':'Click Algernon to offer a grain. / 点击阿尔吉侬，递上一粒食物。'}</p>
      {expanded && <p className="cognition-maze__hint">Drag to orbit · Scroll to zoom · Escape to close<br />拖拽旋转 · 滚轮缩放 · Escape 关闭</p>}
    </aside>
  </div>;
}
