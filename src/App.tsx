...
import { earlyJourneyLabels, earlyJourneySteps, earlyJourneyCopy } from './content/early-journey';
import { JourneyPanel } from './components/narrative/JourneyPanel';
import { JOURNEY_PHASE, type JourneyMilestone } from './core/journey';

import { PERCEPTION_PRESETS } from './audio/perception';
import './app.css';
type Step='chapter'|'realization'|'release'|'empty-room'|'flowers'|'memory'|'title'|'lab'|'test'|'diary'|'preop-report'|'recovery-report'|'bakery'|'gain'|'ascending-report'|'machine'|'revisit'|'peak-report'|'classroom'|'decline'|'last-report'|'finish'|'journey'|'surgery';
...
export default function App(){
 ...
 const [journeyProgress,setJourneyProgress]=useState<ChapterProgress>({completed:[]});
 const [milestone,setMilestone]=useState<JourneyMilestone>('preoperative');
 ...
 const phase:CognitionState['phase']=step==='journey'?JOURNEY_PHASE[milestone]:step==='surgery'?'LOW':step==='chapter'?chapter.phase:['decline','last-report','empty-room'].includes(step)?'DECLINE':['machine','revisit','peak-report','realization','release'].includes(step)?'PEAK':step==='gain'||step==='ascending-report'||step==='classroom'?'ASCENDING':'LOW';
 ...
 const journeyStep=journeyId?earlyJourneySteps.find(item=>item.id===journeyId):undefined;
 const roomScene=step==='journey'&&journeyStep?(journeyStep.sceneId==='classroom'?'classroom':journeyStep.sceneId==='bakery'?'bakery':journeyStep.sceneId==='private-room'?'private-room':'laboratory'):step==='classroom'?'classroom':step==='preop-report'?'private-room':step==='recovery-report'?'laboratory':bakery?'bakery':'laboratory';
 ...
 const go=(next:Step,line?:Pair)=>{stopVoice();setStoryStage(old=>Math.max(old,next==='journey'||next==='bakery'?1:['gain','machine','revisit'].includes(next)?2:next==='chapter'?3:['realization','release','decline','last-report','empty-room'].includes(next)?4:0));setSpeaking(false);setEdges([]);setStep(next);setTaskOpen(next==='journey'||next==='surgery');setTaskView('current');setDialogueOpen(false);if(line)say(line);};
 ...
 const saveDiary=(draft:ReportDraft)=>{const entry=step==='diary'?'first':step==='ascending-report'?'ascending':step==='peak-report'?'peak':step==='last-report'?'last':step==='recovery-report'?'recovery':'preop';const saved={...diaries,[entry]:draft};setDiaries(saved);
   const next:Step=entry==='first'||entry==='recovery'?'journey':entry==='preop'?'surgery':entry==='ascending'?'machine':entry==='peak'?'chapter':'empty-room';
   const nextJourney=entry==='first'?'bakery-low':entry==='recovery'?'bakery-return':journeyId;
   const nextMilestone=entry==='recovery'?'noticing':milestone;
   const nextChapter=entry==='peak'?'algernon-first-failure':chapterId;
   const data:Checkpoint={version:2,savedAt:new Date().toISOString(),step:next==='journey'||next==='surgery'?next:'chapter',chapterId:nextChapter,diaries:saved,observed,bread,choice,intent:entry==='peak'?draft.rawText:intent,milestone:nextMilestone,journeyId:nextJourney};
   const ok=saveCheckpoint(data);if(ok)setCheckpoint({checkpoint:data});else {setAudioNote('Local save failed. Keep this page open. / 本地保存失败，请保持页面打开。');throw new Error('Checkpoint save failed');}
   if(entry==='first'){setJourneyId('bakery-low');setJourneyProgress({completed:[]});bridge('journey',lines.bakery,'The work I know / 我熟悉的活');return;}
   if(entry==='preop'){bridge('surgery',earlyJourneyCopy.surgeryExplanation,'A difficult choice / 一个困难的选择');return;}
   if(entry==='recovery'){setJourneyId('bakery-return');setJourneyProgress({completed:[]});setMilestone('noticing');bridge('journey',earlyJourneyCopy.returnWelcome,'The same bakery / 还是那家面包店');return;}
   if(entry==='last'){setReleased(true);go('empty-room');return;}
   bridge(next,undefined,entry==='peak'?'A familiar route / 熟悉的路线':entry==='ascending'?'One thing moves another / 一件事带动另一件事':'Back to the bakery / 回到面包店',nextChapter);};
 ...
 const resume=()=>{const saved=checkpoint.checkpoint;if(!saved)return;ensureAudio();setStoryStage(saved.step==='journey'?1:saved.step==='machine'?2:saved.step==='chapter'?3:4);setReleased(saved.step==='empty-room');setDiaries(saved.diaries);setObserved(saved.observed);setBread(saved.bread);setChoice(saved.choice);setIntent(saved.intent);setChapterId(saved.chapterId);setLesson({completed:[]});if(saved.version===2&&saved.milestone&&saved.journeyId){setMilestone(saved.milestone);setJourneyId(saved.journeyId);}go(saved.step);};
 const begin=()=>{setStoryStage(0);setReleased(false);setPresented(false);setReversalAsked(false);setConferenceChoice(null);setDiaries({});setLesson({completed:[]});setJourneyProgress({completed:[]});setJourneyId('');setMilestone('preoperative');setObserved([]);setBread(0);setChoice('');setIntent('');ensureAudio();go('lab',lines.welcome);setSpeaker('Dr. Strauss');};
 ...
 const journeyAction=(action:ChapterAction)=>{if(!journeyStep)return;const result=applyChapterAction(journeyStep,journeyProgress,action);if(result.accepted)setJourneyProgress(result.progress);};
 const journeyContinue=()=>{if(!journeyStep)return;const id=journeyStep.id;if(id==='bakery-low'){setJourneyId('learning-low');setJourneyProgress({completed:[]});return;}if(id==='learning-low'){go('preop-report');return;}if(id==='recovery'){go('recovery-report');return;}if(id==='bakery-return'){setJourneyId('learning-return');setJourneyProgress({completed:[]});return;}if(id==='learning-return'){go('ascending-report');return;}};
 ...
 const interact=(rawId:string)=>{
   const id=normalizeHotspot(rawId,roomScene);
   if(transition||taskOpen||dialogueOpen||mazeOpen)return;
   const diaryPhase=['diary','preop-report','ascending-report','peak-report','last-report'].includes(step)||step==='journey'&&journeyId==='recovery';
   const isBook=rawId==='book'||id==='lab-cabinet'||id==='records';
 ...
 return <main className={`game game--${phase.toLowerCase()}`}>
  <div className="world"><ExplorableRoom hotspotLabels={{paper:['diary','preop-report','recovery-report','ascending-report','peak-report','last-report'].includes(step)?'Write diary / 写日记':'Diary / 日记本',test:'Examine test / 查看测试',book:'Inspect book / 看书'}} npcName={roomScene==='classroom'?'Alice / 爱丽丝':roomScene==='bakery'?'Gimpy / 金皮':'Dr. Strauss / 施特劳斯医生'} scene={roomScene} assets={{...getRoomVisuals(roomScene),...(['chapter','realization','release','empty-room','flowers','memory','finish','surgery'].includes(step)?{npc:undefined,npcReaction:undefined}: {})}} onInteract={interact} npcReacting={dialogueOpen&&speaker===(bakery?'Gimpy':'Dr. Strauss')} paused={['title','finish','flowers','memory'].includes(step)||transition||taskOpen||dialogueOpen||mazeOpen}/></div>
  ...
  {step==='journey'&&journeyStep&&<JourneyPanel step={journeyStep} progress={journeyProgress} labels={earlyJourneyLabels} onAction={journeyAction} onContinue={journeyContinue} continueLabel={{en:'Move to the next place',zhHans:'去下一个地方'}}/>}
  {step==='surgery'&&<section className="interaction-panel compact"><p className="kicker">THE OPERATION · 手术</p><h2>{earlyJourneyCopy.surgeryExplanation.en}</h2><p lang="zh-Hans">{earlyJourneyCopy.surgeryExplanation.zhHans}</p><button className="primary" onClick={()=>{setMilestone('recovering');setJourneyId('recovery');setJourneyProgress({completed:[]});bridge('journey',earlyJourneyCopy.recoveryWelcome,'After the operation / 手术之后')}}>I want to try / 我想试试 →</button></section>}
  {['diary','preop-report','recovery-report','ascending-report','peak-report','last-report'].includes(step)&&<section className="interaction-panel report-panel"><DiaryCollage key={step} phase={phase} entryId={step==='diary'?'first':step==='preop-report'?'preop':step==='recovery-report'?'recovery':step==='ascending-report'?'ascending':step==='peak-report'?'peak':'last'} previousText={diaries.peak?.rawText??intent} onPlaceFragment={placeFragment} onSave={saveDiary}/></section>}
