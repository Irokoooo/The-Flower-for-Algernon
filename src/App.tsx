...
import './app.css';
type Step='chapter'|'realization'|'empty-room'|'flowers'|'memory'|'title'|'lab'|'test'|'diary'|'bakery'|'gain'|'machine'|'revisit'|'peak-report'|'classroom'|'decline'|'last-report'|'finish';
...
export default function App(){
 ...
 const phase:CognitionState['phase']=step==='chapter'?chapter.phase:['decline','last-report','empty-room'].includes(step)?'DECLINE':['machine','revisit','peak-report'].includes(step)?'PEAK':step==='gain'||step==='classroom'?'ASCENDING':'LOW';
 ...
 const go=(next:Step,line?:Pair)=>{stopVoice();setStoryStage(old=>Math.max(old,next==='bakery'?1:['gain','machine','revisit'].includes(next)?2:next==='chapter'?3:['realization','decline','last-report','empty-room'].includes(next)?4:0));setSpeaking(false);setEdges([]);setStep(next);setTaskOpen(false);setDialogueOpen(false);if(line)say(line);};
 ...
 const saveDiary=(draft:ReportDraft)=>{const entry=step==='diary'?'first':step==='peak-report'?'peak':'last';const saved={...diaries,[entry]:draft};setDiaries(saved);const next=entry==='first'?'bakery':entry==='peak'?'chapter':'empty-room';const nextChapter=entry==='peak'?'algernon-first-failure':chapterId;const data:Checkpoint={version:1,savedAt:new Date().toISOString(),step:next,chapterId:nextChapter,diaries:saved,observed,bread,choice,intent:entry==='peak'?draft.rawText:intent};const ok=saveCheckpoint(data);if(ok)setCheckpoint({checkpoint:data});else {setAudioNote('Local save failed. Keep this page open. / 本地保存失败，请保持页面打开。');throw new Error('Checkpoint save failed');}if(entry==='last'){setReleased(true);go('empty-room');return;}bridge(next,entry==='first'?lines.bakery:undefined,entry==='peak'?'A familiar route / 熟悉的路线':'Back to the bakery / 回到面包店',nextChapter);};
 ...
 const resume=()=>{const saved=checkpoint.checkpoint;if(!saved)return;ensureAudio();setStoryStage(saved.step==='bakery'?1:4);setReleased(saved.step==='empty-room');setDiaries(saved.diaries);setObserved(saved.observed);setBread(saved.bread);setChoice(saved.choice);setIntent(saved.intent);setChapterId(saved.chapterId);setLesson({completed:[]});go(saved.step);};
 const begin=()=>{setStoryStage(0);setReleased(false);setPresented(false);setDiaries({});setLesson({completed:[]});setObserved([]);setBread(0);setChoice('');setIntent('');ensureAudio();go('lab',lines.welcome);setSpeaker('Dr. Strauss');};
 ...
 const interact=(rawId:string)=>{
   ...
   if(id==='algernon-maze'&&!['chapter','realization','empty-room'].includes(step)){setObserved(a=>a.includes('mouse')?a:[...a,'mouse']);say(observations.mouse);return;}
   if(step==='lab') {
 ...
 const objective=step==='chapter'?chapter.objective.en+' / '+chapter.objective.zhHans:step==='empty-room'?'Look at what remains. Leave when you choose. / 看看留下的东西，想离开时再离开。':step==='realization'?'Return to the records. / 回到记录旁。':step==='classroom'?'Open your lecture notes at the workbench. / 在工作台边打开课堂笔记。':step==='lab'?'Examine two objects, then speak to the researcher. / 观察两件物品后，与研究员交谈。':step==='test'?'Examine the test desk. / 查看测试桌。':['diary','peak-report','last-report'].includes(step)?'Open your notebook at a desk. / 走到桌边打开笔记本。':step==='bakery'?'Go to the bread counter. / 走到面包柜台。':'Inspect the workbench or oven. / 查看工作台或烤炉。';
 return <main className={`game game--${phase.toLowerCase()}`}>
  <div className="world"><ExplorableRoom scene={bakery?'bakery':'laboratory'} assets={{...getRoomVisuals(bakery?'bakery':'laboratory'),...(['chapter','realization','empty-room','flowers','memory','finish'].includes(step)?{npc:undefined,npcReaction:undefined}:{})}} onInteract={interact} npcReacting={dialogueOpen&&speaker===(bakery?'Gimpy':'Dr. Strauss')} paused={['title','finish','flowers','memory'].includes(step)||transition||taskOpen||dialogueOpen||mazeOpen}/></div>
  ...
  {['diary','peak-report','last-report'].includes(step)&&<section className="interaction-panel report-panel"><DiaryCollage key={step} phase={phase} entryId={step==='diary'?'first':step==='peak-report'?'peak':'last'} previousText={diaries.peak?.rawText??intent} onSave={saveDiary}/></section>}
  {step==='chapter'&&<section className="interaction-panel chapter-panel"><p className="kicker">{chapter.sceneId==='classroom'?'LECTURE · 课堂':chapter.sceneId==='conference'?'CONFERENCE · 会议':'RECORDS · 记录'}</p><h2>{chapter.title.en}</h2><p>{chapter.title.zhHans}</p><SubtitleBlock text={chapter.objective}/>
   {chapter.requirements.filter(r=>r.action.kind!=='connect').map(r=>{const ready=(r.after??[]).every(id=>lesson.completed.includes(id));const done=lesson.completed.includes(r.id);const action=r.action;if(action.kind==='place')return <EvidencePlacement key={r.id} itemLabel={chapterObjectLabel(action.itemId)} targetLabel={chapterObjectLabel(action.targetId)} disabled={!ready||done} onPlace={()=>lessonAction(action)}/>;const label=action.kind==='inspect'?'Inspect / 查看 · '+chapterObjectLabel(action.targetId):action.kind==='choose'?'Choose / 选择 · '+chapterObjectLabel(action.optionId):'';return <button className="primary" key={r.id} disabled={!ready||done} onClick={()=>lessonAction(action)}>{done?'✓ ':''}{label}</button>;})}
