...
OUT = ROOT / 'assets/audio/diary-production'
OUT.mkdir(exist_ok=True)


def main():
    plan = json.loads((ROOT / 'assets/audio/diary-voice-jobs.json').read_text(encoding='utf-8'))
    plan['speakers'] = {'charlie': {'voice': 'echo'}}
    env = {}
    ...
    prior = 0  # Separate Lead-reserved diary-murmurs-v1 allocation
    ...
    with OpenAI(api_key=key, base_url='https://api.openai-next.com/v1', max_retries=0, timeout=90,
                http_client=DefaultHttpxClient(follow_redirects=False, timeout=90)) as client:
        for i, job in enumerate(jobs):
            ...
            assert ledger['reservedUSD'] + prior + reserve <= 1
            ...
            try:
                response = client.audio.speech.create(model='tts-1-hd', voice=record['voice'], input=job['text'], response_format='mp3', speed=0.85)
